...
class PlanVc: UIViewController {
    ...
    private func getDataWaitingApprove(){
        ...
        spiner.show(in: view)
        dayOfVm.getWaitingApprovalNextMonth(codeDriver: codeDriver) { (res) in
            switch res {
        ...
        dayOfVm.setPlanDayOff(data: dayOffPlan, codeDriver: codeDriver, idGroup: idGroup) {[weak self] (response) in
            ...
            case .success(_):
                DispatchQueue.main.async {
                    ...
                    self?.getDataWaitingApprove()
                    Helpers().showAlert(view: self!, message: "Please wait approval from admin.".localiz(), customTitle: "Success to set plan next month!".localiz(), customAction1: action1)
                }
            case .failure(let e):
                self?.spiner.dismiss()
    ...
    func showAlert(text: String) {
        ...
        Helpers().showAlert(view: self, message: text,customTitle: "Warning !".localiz(), customAction1: action)
    }
